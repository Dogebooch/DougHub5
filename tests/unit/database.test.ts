...
describe('Database Schema & Initialization', () => {
  ...
  describe('Helper Functions', () => {
    ...
    it('columnExists() detects existing columns', () => {
      expect(columnExists('cards', 'id')).toBe(true)
    ...
    it('columnExists() returns false for missing columns', () => {
      expect(columnExists('cards', 'nonexistent')).toBe(false)
      expect(columnExists('cards', 'foo')).toBe(false)
    })
...
describe('Database Migration v1→v2→v3', () => {
  ...
  it('creates idx_quick_dumps_status index', () => {
    ...
    const indexes = getDatabase()
      .prepare("SELECT name FROM sqlite_master WHERE type='index' AND name='idx_quick_dumps_status'")
      .all()
  ...
  it('creates idx_cards_cardType index', () => {
    initDatabase(dbPath)
    const indexes = getDatabase()
      .prepare("SELECT name FROM sqlite_master WHERE type='index' AND name='idx_cards_cardType'")
      .all()
    expect(indexes).toHaveLength(1)
  })
  ...
  it('creates idx_cards_parentListId index', () => {
    initDatabase(dbPath)
    const indexes = getDatabase()
      .prepare("SELECT name FROM sqlite_master WHERE type='index' AND name='idx_cards_parentListId'")
      .all()
    expect(indexes).toHaveLength(1)
  })
  ...
  it('creates connection indexes (source, target, score)', () => {
    ...
    const sourceIdx = getDatabase()
      .prepare("SELECT name FROM sqlite_master WHERE type='index' AND name='idx_connections_sourceNoteId'")
      .all()
    expect(sourceIdx).toHaveLength(1)
    
    const targetIdx = getDatabase()
      .prepare("SELECT name FROM sqlite_master WHERE type='index' AND name='idx_connections_targetNoteId'")
      .all()
    expect(targetIdx).toHaveLength(1)
    
    const scoreIdx = getDatabase()
      .prepare("SELECT name FROM sqlite_master WHERE type='index' AND name='idx_connections_semanticScore'")
      .all()
  ...
  it('sets new columns to null/default values for existing rows', () => {
    ...
    const cards = getDatabase().prepare('SELECT cardType, parentListId, listPosition FROM cards').all()
  ...
  it('creates backup before migration', () => {
    const { createBackup } = require('@electron/backup-service')
    initDatabase(dbPath)
...
describe('Cards CRUD Operations', () => {
  ...
  it('insert() creates card with all fields', () => {
    const card = createMockCard({ cardType: 'vignette', parentListId: 'list-1', listPosition: 0 })
  ...
  it('update() modifies card fields', () => {
    ...
    cardQueries.update(card.id, { front: 'Updated Question', cardType: 'cloze' })
