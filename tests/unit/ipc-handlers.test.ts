...
import { notifyOllamaStatus } from "@electron/ipc-utils";
...
describe("IPC Handlers - files:saveImage", () => {
  ...
  it("maps mimeTypes correctly", async () => {
    ...
    for (const { mime, ext } of testCases) {
      ...
      expect(
        fs.existsSync(
          path.join(tempUserData, "images", `test-uuid-1234.${ext}`)
        )
      ).toBe(true);
    }
  });
...
describe("IPC Utils - notifyOllamaStatus", () => {
  it("should send IPC message to the main window if it exists", () => {
    ...
    notifyOllamaStatus("starting", "Ollama is starting...");

    expect(mockSend).toHaveBeenCalledWith("ai:ollamaStatus", {
      status: "starting",
      message: "Ollama is starting...",
    });
  });

  it("should not send IPC message if no windows are available", () => {
    vi.mocked(BrowserWindow.getAllWindows).mockReturnValue([]);

    // Should not throw
    expect(() => notifyOllamaStatus("starting", "test")).not.toThrow();
  });

  it("should not send IPC message if the window is destroyed", () => {
    const mockSend = vi.fn();
    const mockWin = {
    ...
    notifyOllamaStatus("starting", "test");
