...
describe('QuickCaptureModal', () => {
  ...
  beforeEach(() => {
    ...
    mockFilesSaveImage.mockResolvedValue({ data: { path: 'images/test.png' }, error: null });
  });

  it('renders textarea with correct placeholder when open', () => {
    render(<QuickCaptureModal isOpen={true} onClose={mockOnClose} />);
    expect(screen.getByPlaceholderText(/Paste text or image.../i)).toBeDefined();
  });
  ...
  it('saves text capture and truncates title', async () => {
    ...
    await waitFor(() => {
      expect(mockSourceItemsCreate).toHaveBeenCalledWith(expect.objectContaining({
  ...
  it('clears image when X button is clicked', async () => {
    ...
    fireEvent.paste(screen.getByPlaceholderText(/Paste text or image.../i), { clipboardData });
