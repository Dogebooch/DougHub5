...
describe("InboxView", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    // Mock store with proper selector support
    ...
    (window as any).api = {
      sourceItems: {
        getByStatus: vi
          .fn()
          .mockResolvedValue({ data: mockSourceItems, error: null }),
        delete: vi.fn().mockResolvedValue({ data: true, error: null }),
      },
      db: {
        status: vi.fn().mockResolvedValue({
          data: { inboxCount: 3, queueCount: 0 },
          error: null,
        }),
      },
    };
  ...
  it("shows empty state when no items found", async () => {
    (window as any).api.sourceItems.getByStatus.mockResolvedValue({
      data: [],
      error: null,
    });
