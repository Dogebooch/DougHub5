...
const BasicLearningStepsStrategy = (params2, state, cur_step) => {
...
class FSRSAlgorithm {
  ...
  update_parameters(params2) {
    const _params = generatorParameters(params2);
...
function toFSRSCard(card) {
  return {
    ...
    learning_steps: 0,
    // Default to 0, will be set by FSRS
    reps: card.reps,
    lapses: card.lapses,
    state: card.state,
...
function scheduleReview(cardId, rating, reviewTime = /* @__PURE__ */ new Date()) {
  ...
  const fsrsCard = dbCard.state === 0 ? createEmptyCard(reviewTime) : toFSRSCard(dbCard);
  ...
  const updatedCard = { ...dbCard, ...fsrsUpdates };
  const reviewLog = {
    id: randomUUID(),
    cardId,
    rating,
...
function getIntervalPreviews(cardId) {
  const cards = cardQueries.getAll();
  const dbCard = cards.find((c) => c.id === cardId);
  if (!dbCard) {
    throw new Error(`Card not found: ${cardId}`);
  ...
  const fsrsCard = dbCard.state === 0 ? createEmptyCard(now) : toFSRSCard(dbCard);
