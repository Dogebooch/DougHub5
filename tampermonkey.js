...
(function() {
    ...
    function shouldShowButton() {
        ...
        if (site === 'ACEP PeerPrep') {
            // Check for quiz results page URL pattern (completed quiz with answers)
            const isQuizResultsPage = url.includes('/quiz/') && url.includes('testAttemptArchiveId');
            // Check for individual question review page (has Key Point text)
            const hasKeyPoint = bodyContent.includes('Key Point');
            // Check for answer explanation content
            const hasAnswerExplanation = bodyContent.includes('Correct Answer') ||
                                         bodyContent.includes('Your Answer') ||
                                         !!document.querySelector('.answer-explanation, .feedback, [class*="explanation"]');

            return hasKeyPoint || isQuizResultsPage || hasAnswerExplanation;
        } else if (site === 'MKSAP 19') {
