...
// ============================================================================
// Practice Bank Flashcard System (v28)
// ============================================================================

/**
 * Card types for Practice Bank, mapping to the card templates in the philosophy doc.
 * Each archetype has specific card types that can be generated.
 */
export type PracticeBankCardType =
  // Universal
  | "golden_ticket"
  // Illness Script
  | "pathophysiology"
...
/**
 * Maturity states for Practice Bank cards
 * - new: Never reviewed
 * - learning: In early learning phase (state < 2)
 * - reviewing: In active review (state == 2)
 * - mature: Interval >= 60 days
 * - retired: Graduated, suspended from queue
 */
export type PracticeBankMaturityState =
  | "new"
  | "learning"
  | "reviewing"
  | "mature"
...
/**
 * Practice Bank Flashcard - the core flashcard entity.
 * Distinct from DbCard (the old system) - this is specifically for
 * the archetype-based Practice Bank workflow.
 */
export interface DbPracticeBankFlashcard {
  id: string;
  entityId: string;
  ...
  // Maturity Tracking
  maturityState: PracticeBankMaturityState;
...
/**
 * Row type for SQLite (boolean -> integer conversion)
 */
export interface PracticeBankFlashcardRow {
  id: string;
  entityId: string;
  cardType: string;
  front: string;
  back: string;
  isGoldenTicket: number; // 0 or 1
...
/**
 * Simulator Attempt - tracks vignette-based testing
 */
export interface DbSimulatorAttempt {
  id: string;
  entityId: string;
  vignetteText: string;
  userAnswer: string | null;
  ...
  isCorrect: boolean;
  failureAttributions: string[] | null; // Which checkboxes user selected
  attemptedAt: string;
}
 ...
 * Row type for SQLite
 */
export interface SimulatorAttemptRow {
  id: string;
  entityId: string;
  vignetteText: string;
  userAnswer: string | null;
  ...
  failureAttributions: string | null; // JSON string
  attemptedAt: string;
}

/**
 * Card generation template - defines how to generate a card from an entity
 */
export interface CardGenerationTemplate {
  cardType: PracticeBankCardType;
  frontTemplate: string;
  backField: string;
  isGoldenTicket: boolean;
  priority: number; // Lower = generated first
  conditionalField?: string; // Only generate if this field exists
}

/**
 * Failure attribution checklist item
 */
export interface FailureAttributionOption {
  id: string;
  label: string;
  linkedCardType: PracticeBankCardType;
}

/**
 * Archetype-specific failure attribution checklists
 */
export const FAILURE_ATTRIBUTION_OPTIONS: Record<
  KnowledgeEntityType,
