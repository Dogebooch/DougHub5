...
const ILLNESS_SCRIPT_TEMPLATES: CardGenerationTemplate[] = [
  {
...
const DRUG_TEMPLATES: CardGenerationTemplate[] = [
  {
    cardType: "golden_ticket",
...
const PATHOGEN_TEMPLATES: CardGenerationTemplate[] = [
  {
    cardType: "golden_ticket",
...
const PRESENTATION_TEMPLATES: CardGenerationTemplate[] = [
  {
    cardType: "golden_ticket",
    frontTemplate:
      "What are the immediate life threats to consider with {title}?",
...
const IMAGING_TEMPLATES: CardGenerationTemplate[] = [
  {
    cardType: "golden_ticket",
  ...
  },
  {
    cardType: "next_step",
    frontTemplate:
      "What is the next step after finding {title}? (Urgency: {urgency})",
    backField: "associatedConditions",
    isGoldenTicket: false,
    priority: 3,
...
const DIAGNOSTIC_TEMPLATES: CardGenerationTemplate[] = [
  {
    cardType: "golden_ticket",
...
const PROCEDURE_TEMPLATES: CardGenerationTemplate[] = [
  {
    cardType: "golden_ticket",
...
const ANATOMY_TEMPLATES: CardGenerationTemplate[] = [
  {
    cardType: "golden_ticket",
...
const ALGORITHM_TEMPLATES: CardGenerationTemplate[] = [
  {
    cardType: "golden_ticket",
...
const GENERIC_TEMPLATES: CardGenerationTemplate[] = [
  {
    cardType: "golden_ticket",
 ...
 */
export function generateCardsForEntity(
  ...
  for (const template of templates) {
    ...
    let front = template.frontTemplate
      ...
      .replace(
        /\{urgency\}/g,
        String(entity.structuredData?.urgency || "routine"),
      );
    ...
    if (isGoldenTicket && hasAudio) {
      ...
      front = `Identify this sound:\n[AUDIO: ${(entity as unknown as { audioFilePath?: string }).audioFilePath}]`;
    }

    const card: DbPracticeBankFlashcard = {
      id: uuidv4(),
      entityId: entity.id,
      cardType,
      front,
 ...
 */
function formatBackValue(value: unknown): string {
 ...
 */
export function getExpectedCardCount(entity: DbKnowledgeEntity): {
  goldenTicket: number;
  practiceBank: number;
} {
  const templates = ARCHETYPE_TEMPLATES[entity.entityType];
  if (!templates) return { goldenTicket: 0, practiceBank: 0 };
 ...
 */
export function isEntityReadyForForging(entity: DbKnowledgeEntity): boolean {
  const goldenTicketField = GOLDEN_TICKET_FIELDS[entity.entityType];
  if (!goldenTicketField) return false;
