...
exports.knowledgeEntityLinkQueries = exports.knowledgeEntityQueries = void 0;
const db_connection_1 = require("./db-connection");
const types_1 = require("./types");
// ============================================================================
// Entity CRUD Operations
// ============================================================================
exports.knowledgeEntityQueries = {
    /**
     * Get all knowledge entities
     */
    getAll() {
        const stmt = (0, db_connection_1.getDatabase)().prepare("SELECT * FROM knowledge_entities ORDER BY title");
        const rows = stmt.all();
     ...
     */
    getById(id) {
        const stmt = (0, db_connection_1.getDatabase)().prepare("SELECT * FROM knowledge_entities WHERE id = ?");
    ...
    getByType(entityType) {
        const stmt = (0, db_connection_1.getDatabase)().prepare("SELECT * FROM knowledge_entities WHERE entityType = ? ORDER BY title");
     ...
     */
    getByDomain(domain) {
        const stmt = (0, db_connection_1.getDatabase)().prepare(`SELECT * FROM knowledge_entities WHERE domains LIKE ? ORDER BY title`);
     ...
     */
    getChildren(parentEntityId) {
        const stmt = (0, db_connection_1.getDatabase)().prepare("SELECT * FROM knowledge_entities WHERE parentEntityId = ? ORDER BY title");
        const rows = stmt.all(parentEntityId);
        return rows.map(parseEntityRow);
    },
    /**
    ...
    getAncestors(entityId) {
        const ancestors = [];
        let current = exports.knowledgeEntityQueries.getById(entityId);
        while (current?.parentEntityId) {
            const parent = exports.knowledgeEntityQueries.getById(current.parentEntityId);
            if (parent) {
     ...
     */
    update(id, updates) {
        const current = exports.knowledgeEntityQueries.getById(id);
        if (!current) {
     ...
     */
    delete(id) {
        const stmt = (0, db_connection_1.getDatabase)().prepare("DELETE FROM knowledge_entities WHERE id = @id");
        stmt.run({ id });
    },
    /**
    ...
    `;
        ...
        const stmt = (0, db_connection_1.getDatabase)().prepare(query);
     ...
     */
    findExactDuplicate(title, entityType) {
        const stmt = (0, db_connection_1.getDatabase)().prepare(`SELECT * FROM knowledge_entities
       WHERE LOWER(title) = LOWER(?) AND entityType = ?`);
        const row = stmt.get(title.trim(), entityType);
        return row ? parseEntityRow(row) : null;
    },
    /**
     * Set the Golden Ticket value (user completing the required field)
     */
    setGoldenTicketValue(id, value) {
        const now = new Date().toISOString();
        const stmt = (0, db_connection_1.getDatabase)().prepare(`
     ...
     */
    revealAiHint(id) {
        const entity = exports.knowledgeEntityQueries.getById(id);
...
// ============================================================================
exports.knowledgeEntityLinkQueries = {
    /**
     ...
     */
    getLinkedByType(entityId, linkType) {
        const stmt = (0, db_connection_1.getDatabase)().prepare(`
      SELECT ke.* FROM knowledge_entities ke
      JOIN knowledge_entity_links kel ON
    ...
    `);
        const rows = stmt.all(entityId, linkType, entityId, linkType);
        return rows.map(parseEntityRow);
    },
    /**
     * Create a link between two entities
     */
    linkEntities(id, sourceEntityId, targetEntityId, linkType) {
        const now = new Date().toISOString();
        const stmt = (0, db_connection_1.getDatabase)().prepare(`
     ...
     * Remove a link
     */
    unlinkEntities(linkId) {
        const stmt = (0, db_connection_1.getDatabase)().prepare("DELETE FROM knowledge_entity_links WHERE id = ?");
        stmt.run(linkId);
    },
    /**
     * Remove all links between two specific entities
     */
    unlinkByEntities(entityId1, entityId2) {
        const stmt = (0, db_connection_1.getDatabase)().prepare(`
     ...
     */
    linkExists(sourceEntityId, targetEntityId, linkType) {
        let query = `
