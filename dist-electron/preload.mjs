...
const api = {
  ...
  backup: {
    ...
    getLastTimestamp: () => electron.ipcRenderer.invoke("backup:getLastTimestamp"),
    create: () => electron.ipcRenderer.invoke("backup:create"),
    selectFile: () => electron.ipcRenderer.invoke("backup:selectFile"),
    restore: (filePath) => electron.ipcRenderer.invoke("backup:restore", filePath),
    cleanup: (retentionDays) => electron.ipcRenderer.invoke("backup:cleanup", retentionDays),
    onAutoComplete: (callback) => {
      const subscription = (_event, timestamp) => callback(timestamp);
      electron.ipcRenderer.on("backup:auto-complete", subscription);
      return () => electron.ipcRenderer.removeListener("backup:auto-complete", subscription);
    }
  },
  db: {
